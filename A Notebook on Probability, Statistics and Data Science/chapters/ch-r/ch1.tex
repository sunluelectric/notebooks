...
To access the element in a vector, use \verb|<vector>[<index>]|. Again, notice that the first element in a vector has the index of 1 instead of 0. The index can be an integer, or a list of integer such as a sequence. Examples are given below.
\begin{lstlisting}
> s <- c("a", "b", "c", "d", "e", "f", "g")
> s[1]
[1] "a"
...
\subsection{Building Data Frames}
...
\section{Data Preparation} \label{ch:r1:sec:datapreparation}
