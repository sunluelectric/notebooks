...
\textit{Kubernetes}, also known as \textit{k8s}, is an open-source container orchestration system originally developed by Google. It automates the deployment, scaling, and management of containerized applications.
...
Nowadays, many cloud service providers also support Kubernetes-based container deployment, such as Elastic Kubernetes Service (EKS) by AWS and Google Kubernetes Engine (GKE) by Google Cloud. Notice that these managed services often come with some unique features built on top of Kubernetes, making the using of Kubernetes on these platform not entirely the same as (often easier than) doing it on a local machine.

Nevertheless, the majority part of this chapter focuses on introducing running Kubernetes on a local server, where \verb|kubectl| is used as the user interface to Kubernetes. Additionally, Minikube is used to host Kubernetes. Minikube is an open-source software developed by the Kubernetes community to start a VM and run a single-node Kubernetes cluster on a local machine, and it suits well for training and testing.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=350pt]{chapters/part-3/figures/k8sarchitecture.png}
...
The installation guidance of Kubernetes can be found at its official website \textit{kubernetes.io}. Notice that different OSs adopt different ways of installing and using Kubernetes. In this notebook only Linux OS is concerned.

As introduced earlier, \textit{kubectl} is used to interact with Kubernetes. For illustration purpose we also use Minikube to setup a small Kubernetes cluster in the local machine. They can be installed separately. See following links for more details.
\begin{lstlisting}
https://kubernetes.io/docs/tasks/tools/install-kubectl-linux/
https://minikube.sigs.k8s.io/docs/start/
\end{lstlisting}

Notice that when both Minikube and \textit{kubectl} are installed, Minikube needs to be started before running \textit{kubectl} commands. Start Minikube using
\begin{lstlisting}
$ minikube start
\end{lstlisting}
Upon the start of Minikube, use the following command to verify the successful running of \textit{kubectl}.
\begin{lstlisting}
...
Notice that Minikube also comes with a built-in \verb|kubectl|. It is possible to use that \verb|kubectl| instead of installing one separately. To very the existence of the built-in \verb|kubectl|, start Minikube first and then use
\begin{lstlisting}
$ minikube kubectl -- cluster-info
Kubernetes control plane is running at https://192.168.49.2:8443
CoreDNS is running at https://192.168.49.2:8443/api/v1/namespaces/kube-system/services/kube-dns:dns/proxy
\end{lstlisting}
where \verb|--| in the command tells the shell that all the followed arguments shall be passed to \verb|kubectl|, but not \verb|minikube|. For convenience, one may want to use alias as follows
\begin{lstlisting}
...
\subsection{Brief Introduction to Kubernetes Configuration}

Kubernetes configuration files are used to guide Kubernetes on how it should manage the project. 
...
As examples, commonly seen templates of Kubernetes configuration files are given below. For \verb|Pod|,
\begin{lstlisting}
apiVersion: <version> # e.g., v1
kind: Pod
metadata:
  name: <pod-name>
...
For \verb|Deployment|,
\begin{lstlisting}
apiVersion: <version> # e.g., apps/v1
kind: Deployment
metadata:
...
\begin{itemize}
	\item \verb|apiVersion| tells the API version.
	
	Kubernetes groups APIs into different collections based on their roles. For example, \verb|v1| belongs to the core API group, inside which the basic objects such as \verb|Pod| are defined. On the other hand, \verb|apps/v1| belongs to the application API group where application-relevant services such as \verb|Deployment| are defined.
	
	The user needs to choose the proper API version for the object type so that Kubernetes can interpret it correctly.
	
	\item \verb|kind| indicates the type of the object that the configuration file describes.
	
	\item \verb|metadata| indicates the name and labels of the object. 
	
	\item \verb|port|, \verb|targetPort| and \verb|nodePort| are used in \verb|kind:Service| to specify ports, where \verb|nodePort| is the port the node exposes to the external world, \verb|port| the port of the Service itself, and \verb|targetPort| the port of the pods where the message will be forwarded to.
\end{itemize}

There are many different kinds of objects. Commonly used Kubernetes object types are summarized in Table \ref{ch:vac:tab:objtype}.
...
Composing Kubernetes configuration files is a key part in Kubernetes-based application deployment. This section only gives a glance of what the configuration files look like. More details are given in later sections of the chapter.

If Kubernetes is deployed on the cloud using managed services such as EKS and GKE, the Kubernetes configuration files needs to be slightly modified. Still, it is easier to migrate Kubernetes-based applications across platforms since the modification is minor than if proprietary services such as ECS were used.
...
With the above been done, open a browser and use \verb|<ip>:<port>| to access the application running in the container, where \verb|<ip>| is the IP address of the VM that hosts Kubernetes, and \verb|<port>| the port configured in NodePort service under \verb|nodePort|. If Minikube is used to create the VM that hosts Kubernetes, the IP address can be found by running \verb|minikube ip|.
...
\end{lstlisting}
and Kubernetes will try to apply all the configuration files in that directory.
...
Without container orchestration tools such as Kubernetes, one of the most challenging tasks is to update the container for a different configuration, for example, changing the underlying image. 
